...
export default function Marquee() {
  ...
  return (
    <div className="relative overflow-hidden border-y border-line bg-void/40 py-5">
      ...
      <div className="flex w-max animate-marquee">
        {row.map((item, i) => (
          <div key={`${item}-${i}`} className="flex items-center">
            <span className="whitespace-nowrap px-8 text-[12px] uppercase tracking-[0.2em] text-muted">
              {item}
            </span>
            <span className="size-1 rotate-45 bg-accent-dim" />
          </div>
        ))}
      </div>
    </div>
